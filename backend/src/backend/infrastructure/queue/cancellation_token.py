...
class CancellationToken:
    ...
    between atomic steps. If the job row is gone, CancelledByUserError is raised.
    """
    ...
    def check(self) -> None:
        """Raise CancelledByUserError if the job has been deleted (cancelled)."""
        if not self._job_repo.job_exists(self._job_id):
            raise CancelledByUserError(self._job_id)
