...
Статусы фоновых операций — через механизм джоб (arq), а не через колонки в БД. Если операция выполняется асинхронно воркером — её прогресс отслеживается через статус джобы. Не добавлять `*_status`-колонки для отслеживания прогресса фоновых задач.
...
```
make logs         # app + worker + redis + ai_proxy (текущая рабочая копия)
```

Перезапуск воркера (в обход пересборки всего compose):

```
docker compose restart worker    # из корня нужной рабочей копии
```
