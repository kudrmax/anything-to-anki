"""Diagnostic: analyze word list distribution by CEFR level and Zipf frequency.

Uses the same VotingCEFRClassifier and WordfreqFrequencyProvider as production.

Usage (from project root):
    # From DB (known words):
    source .venv/bin/activate && python scripts/analyze_known_words.py

    # From TSV file (e.g. Anki dump):
    source .venv/bin/activate && python scripts/analyze_known_words.py --input scripts/anki_learned_words.tsv
"""
...
def load_known_words() -> list[tuple[str, str]]:
    conn = sqlite3.connect(f"file:{DB_PATH}?mode=ro", uri=True)
    rows = conn.execute("SELECT lemma, pos FROM known_words").fetchall()
    conn.close()
    return rows


def load_words_from_tsv(path: Path) -> list[tuple[str, str]]:
    """Load lemma/pos pairs from a TSV file (lemma<TAB>pos per line)."""
    ...
    with open(path) as f:
        for line in f:
            ...
            parts = line.split("\t")
            lemma = parts[0]
            pos = parts[1] if len(parts) > 1 else "VERB"
...
def enrich_words(
    ...
    for lemma, pos in words:
        breakdown = classifier.classify_detailed(lemma, pos)
        cefr = breakdown.final_level.name
        zipf = freq.get_zipf_value(lemma)
        result.append(WordInfo(
            lemma=lemma, pos=pos, cefr=cefr, zipf=zipf,
...
def report_overall(words: list[WordInfo]) -> None:
    ...
    print(f"  Total known words:     {total}")
...
def report_cefr_distribution(words: list[WordInfo]) -> None:
    ...
    if below:
        ...
        print(f"    (these shouldn't normally appear in runtime)")
...
def report_anomalies(words: list[WordInfo]) -> None:
    ...
    print(f"\n  5a. Known words with Zipf < {LOW_ZIPF_THRESHOLD} (rare but known):")
    if low_zipf:
        for w in low_zipf:
            print(f"      {w.lemma:<25s} {w.pos:<6s} CEFR={w.cefr}  Zipf={w.zipf:.2f}  ({w.decision_method})")
    else:
        print(f"      (none)")

    unknown = sorted([w for w in words if w.cefr == "UNKNOWN"], key=lambda w: w.lemma)
    print(f"\n  5b. Known words without CEFR level ({len(unknown)}):")
    if unknown:
        for w in unknown:
            zipf_note = f"Zipf={w.zipf:.2f}" if w.zipf > 0 else "not in wordfreq"
            print(f"      {w.lemma:<25s} {w.pos:<6s} {zipf_note}")
    else:
        print(f"      (none)")
    ...
    print(f"\n  5c. Known words at or below user level ({USER_CEFR_LEVEL}):")
    if below_level:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze word list by CEFR/Zipf")
    parser.add_argument(
        "--input", type=Path, default=None,
        help="TSV file with lemma<TAB>pos per line (default: read from DB known_words)",
    )
    ...
    if args.input:
        ...
        raw_words = load_words_from_tsv(args.input)
    else:
        if not DB_PATH.exists():
            print(f"ERROR: DB not found at {DB_PATH}")
            return
        print(f"Source: {DB_PATH} (known_words table)")
        raw_words = load_known_words()
    ...
    words = enrich_words(raw_words, classifier, freq)
