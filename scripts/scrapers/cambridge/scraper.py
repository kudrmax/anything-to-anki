...
# Based on https://github.com/Blackdeer1524/CambridgeDict.py (MIT License)
# Copyright (c) 2021 Blackdeer1524
...
class RateLimitError(Exception):
    """Raised when Cambridge returns 403 (captcha/rate limit)."""
...
def get_tags(tags_section: Optional[bs4.Tag]) -> tuple[LEVEL_T,
                                                       ...
                                                       USAGES_T,
                                                       DOMAINS_T]:
    def find_tag(html_tag: str, params: dict[str, str]) -> str:
    ...
    def find_all_tags(html_tag: str, params: dict[str, str]) -> list[str]:
        nonlocal tags_section
...
def get_phonetics(
    ...
    for child in ipa:
        ...
        if ipa_parent is None:
            ipa_parent = prev_ipa_parent
        else:
            prev_ipa_parent = ipa_parent
...
def update_word_dict(word_dict:              RESULT_FORMAT,
                     ...
                     us_audio_links:         Optional[US_AUDIO_LINKS_T]         =None) -> None:
...
def define(word: str,
    ...
    if page.status_code == 403:
        raise RateLimitError(f"403 Forbidden for '{word}' (likely captcha/rate limit)")
